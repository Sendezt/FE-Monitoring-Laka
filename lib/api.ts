...
export const authApi = {
  login: (username: string, password: string) =>
    api.post<ApiResponse<{ token: string; user: AuthUser }>>(
      "/api/auth/login",
      { username, password },
    ),
};
...
export interface RekapRow {
  ...
  terjamin: number;
  eg2r: number;
  tidak_terjamin: number;
  kt_depan_depan: number;
  kt_depan_samping: number;
  kt_depan_belakang: number;
  kt_belakang_samping: number;
  kt_samping_samping: number;
  kt_beruntun: number;
  kt_ka: number;
  kt_pjk: number;
  kt_jatuh_sendiri: number;
  kt_ka_pjk: number;
  pf_pelajar: number;
  pf_karyawan: number;
  pf_wiraswasta: number;
  pf_pns: number;
  pf_pedagang: number;
  pf_buruh: number;
  pf_pensiunan: number;
  pf_guru: number;
  pf_petani: number;
  pf_rumah_tangga: number;
  pf_tidak_bekerja: number;
  pf_supir: number;
  pf_lainnya: number;
  jk_a: number;
  jk_b: number;
  ...
  jk_sepeda: number;
  jk_pjk: number;
  jk_tabrak_lari: number;
  telat_1_3: number;
  telat_4_7: number;
  telat_lebih_7: number;
}
...
export const laporanApi = {
  list: (params?: {
    ...
    api.get<PaginatedResponse<LaporanPolisi>>("/api/laporan-polisi", {
      params,
    }),
  get: (id: number) =>
    api.get<ApiResponse<LaporanPolisi>>(`/api/laporan-polisi/${id}`),
  create: (data: CreateLaporanPayload) =>
    api.post<ApiResponse<LaporanPolisi>>("/api/laporan-polisi", data),
  update: (id: number, data: Partial<CreateLaporanPayload>) =>
    api.put<ApiResponse<LaporanPolisi>>(`/api/laporan-polisi/${id}`, data),
  delete: (id: number) =>
    api.delete<ApiResponse<null>>(`/api/laporan-polisi/${id}`),
  statistikKomparasi: (p: {
    start1: string;
    end1: string;
    start2: string;
    end2: string;
  }) =>
    api.get<ApiResponse<unknown>>("/api/laporan-polisi/statistik/komparasi", {
      params: p,
    }),
  statusLp: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<
      ApiResponse<{
        total: number;
  ...
  statistikKeterjaminan: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<KeterjaminanCardData>>(
      "/api/laporan-polisi/statistik/keterjaminan",
      { params },
    ),
  rekapitulasiPolres: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<{ rows: RekapRow[]; totals: RekapRow }>>(
      "/api/laporan-polisi/rekapitulasi/polres",
      { params },
    ),
  rekapitulasiLoket: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<{ rows: RekapRow[]; totals: RekapRow }>>(
      "/api/laporan-polisi/rekapitulasi/loket",
      { params },
    ),
};
...
export const chartApi = {
  totalLakaPerWilayah: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<TotalLakaPerWilayahData>>(
      "/api/chart/statistik/total-laka-per-wilayah",
      { params },
    ),
  totalKorbanPerWilayah: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<TotalKorbanPerWilayahData>>(
  ...
  korbanPerProfesi: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<ProfesiKorbanData>>(
      "/api/chart/statistik/korban-per-profesi",
      { params },
    ),
  korbanPerJenisKendaraan: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<JenisKendaraanKorbanData>>(
      "/api/chart/statistik/korban-per-jenis-kendaraan",
      { params },
    ),
  topKecamatanLaka: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<TopKecamatanLakaItem[]>>(
      "/api/chart/statistik/top-20-kecamatan-laka",
      { params },
    ),
  topRumahSakitKorban: (params?: {
    from?: string;
    to?: string;
    polres_id?: string | number;
  }) =>
    api.get<ApiResponse<TopRumahSakitKorbanItem[]>>(
  ...
  trendHarian: (params: {
    ...
    api.get<ApiResponse<TrendHarianData>>("/api/chart/statistik/trend-harian", {
      params,
    }),
};
...
export const migrasiApi = {
  ...
  check: (payload: MigrasiPayload) =>
    api.post<ApiResponse<MigrasiCheckResult>>("/api/migrasi/check", {
      payload,
    }),
  import: (
    payload: MigrasiPayload,
    options?: { createMissingMaster?: boolean },
  ) =>
    api.post<ApiResponse<LaporanPolisi>>("/api/migrasi/import", {
      payload,
      createMissingMaster: options?.createMissingMaster ?? false,
    }),
};
...
const masterGet = (url: string) => () =>
  api.get<ApiResponse<MasterItem[]>>(url);
...
const masterCrud = (baseUrl: string) => ({
  list: (params?: number | Record<string, unknown>) => {
    const normalized =
      typeof params === "number" ? { wilayah_id: params } : (params ?? {});
    return api.get<ApiResponse<MasterItem[]>>(baseUrl, {
      params: withPaging(normalized as Record<string, unknown>),
    });
  },
  get: (id: number) => api.get<ApiResponse<MasterItem>>(`${baseUrl}/${id}`),
  create: (data: { nama: string }) =>
    api.post<ApiResponse<MasterItem>>(baseUrl, data),
  update: (id: number, data: { nama: string }) =>
    api.put<ApiResponse<MasterItem>>(`${baseUrl}/${id}`, data),
...
export const masterApi = {
  ...
  polres: {
    ...
    listByWilayah: (wilayahId: number, params?: Record<string, unknown>) =>
      api.get<ApiResponse<MasterItem[]>>(`/api/polres/wilayah/${wilayahId}`, {
        params: withPaging(params ?? {}),
      }),
  },
  kecamatan: {
    ...masterCrud("/api/kecamatan"),
    listByPolres: (polresId: number, params?: Record<string, unknown>) =>
      api.get<ApiResponse<MasterItem[]>>(`/api/kecamatan/polres/${polresId}`, {
        params: withPaging(params ?? {}),
      }),
  },
