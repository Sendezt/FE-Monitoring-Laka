...
const EXTERNAL_API_URL = "https://be-monitoring-laka.vercel.app/api/card/terjamin";
...
export async function GET() {
  try {
    ...
    if (parsedCount === null) {
      return NextResponse.json(
        { success: false, error: "Invalid data format received from upstream terjamin API", raw: data },
        { status: 502 }
      );
    }
    ...
    console.error("[API Route terjamin] Fetch error:", error?.message || error);
    return NextResponse.json(
      {
        success: false,
