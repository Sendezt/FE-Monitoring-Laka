...
const EXTERNAL_API_URL = "https://be-monitoring-laka.vercel.app/api/card/lpterlambat";
...
export async function GET() {
  try {
    ...
    if (parsed === null) {
      return NextResponse.json(
        { success: false, error: "Invalid data format received from upstream lpterlambat API", raw: data },
        { status: 502 }
      );
    }
    ...
    console.error("[API Route lpterlambat] Fetch error:", error?.message || error);
    return NextResponse.json(
      {
        success: false,
