...
const EXTERNAL_API_URL = "https://be-monitoring-laka.vercel.app/api/card/llmd";
...
export async function GET() {
  try {
    ...
    if (parsedCount === null) {
      return NextResponse.json(
        { success: false, error: "Invalid data format received from upstream llmd API", raw: data },
        { status: 502 }
      );
    }
    ...
    console.error("[API Route llmd] Fetch error:", error?.message || error);
    return NextResponse.json(
      {
        success: false,
