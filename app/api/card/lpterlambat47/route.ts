...
const EXTERNAL_API_URL = "https://be-monitoring-laka.vercel.app/api/card/lpterlambat47";
...
export async function GET() {
  try {
    ...
    if (parsed === null) {
      return NextResponse.json(
        { success: false, error: "Invalid data format received from upstream lpterlambat47 API", raw: data },
        { status: 502 }
      );
    }
    ...
    console.error("[API Route lpterlambat47] Fetch error:", error?.message || error);
    return NextResponse.json(
      {
        success: false,
